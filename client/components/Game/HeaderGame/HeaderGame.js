...
function Info(props) {
    ...
    const addFavorite = async () => {
        if(auth) {
            toast.warning("Agregando a lista de favoritos");
            await addFavoriteApi(auth.idUser, game.id, logout);
            setReloadFavorite(true);
        } else {
            toast.warning("Necesita loguearse para agregar a favoritos");
        }
    }

    const deleteFavorite = async () => {
        if(auth) {
            toast.warning("Eliminando de la lista de favoritos");
            await deleteFavoriteApi(auth.idUser, game.id, logout);
            setReloadFavorite(true);
        } else {
            toast.warning("Necesita loguearse para eliminar de favoritos");
        }
    }
